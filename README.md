...
Identity templates and presets for the [Architect](https://github.com/Decalyst) agent harness.

Each Architect project runs a multi-agent pipeline — planner, tiered coders, reviewer, researcher, memory, fixer. Every agent has a **persona** that shapes how it writes, reviews, plans, or investigates. This repo holds the defaults.
...
## How Architect consumes these files

Architect ships with **hardcoded sensible defaults** for every agent. It does not fetch this repo at runtime. These files exist for two purposes:

1. **Chat-triggered download.** When a user asks Architect to "set up identities" / "customize agent personas" / similar, the harness downloads `templates/` into the project's `.architect/agents/` folder for the user to edit.
2. **Settings page download.** A button in Architect's settings lets the user download templates (blank) or presets (opinionated defaults) into the project.
...
### Resolution order inside Architect
